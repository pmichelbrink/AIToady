...
namespace AIToady.Harvester.ViewModels
{
    ...
    /// </summary>
    public class RokslideViewModel : BaseViewModel
    {
        public RokslideViewModel()
        {
            MessagesPerPage = 15;
        }
        ...
        //        string script = @"
        //            let threads = [];
        //            document.querySelectorAll('.structItem-title a[href*=""/threads/""]').forEach(anchor => {
        //                let structItem = anchor.closest('.structItem');
        //                let latestDate = null;
        //                if (structItem) {
        ...
        //                    }
        //                }
        //                threads.push({ url: anchor.href, lastPostDate: latestDate });
        //            });
        ...
        protected async override Task<List<ForumMessage>> HarvestPage()
        {
            ...
            string extractScript = $@"
                ...
                document.querySelectorAll('{messageSelector}').forEach(messageDiv => {{
                    ...
                    let timeElement = messageDiv.closest('article')?.querySelector('.u-dt') || messageDiv.querySelector('.u-dt');
                    let images = [];
                    let attachments = [];
